...
void GLKLib::screen_to_wcl_new(double sx, double sy, double sz, double& cx, double& cy, double& cz)
{
    GLdouble objx, objy, objz;
    qreal deviceRatio = devicePixelRatio();
    double x = sx * (double)deviceRatio;
    double y = m_SizeY - sy * (double)deviceRatio;

    gluUnProject(x, y, sz, modelMatrix, projMatrix, viewport, &objx, &objy, &objz);
...
void GLKLib::wcl_to_screen_new(double cx, double cy, double cz, double& sx, double& sy, double& sz)
{
    GLdouble winx, winy, winz;
...
void GLKLib::draw_line_2d()
{
    ...
    for (int k = 0; k < 21; k++) {
        painter.drawLine(centerPoint_.at(k), centerPoint_.at(k+1));
    }
